...
import { getAllFeedback } from '../../../redux/APIs/slices/feedbackSlice';
import { FaStar, FaUser, FaCalendar, FaClock, FaEye, FaFilter } from 'react-icons/fa';
...
const AdminFeedback = () => {
  ...
  const { allFeedback, loading, error } = useSelector(state => state.feedbackSlice);
  ...
  }, [dispatch, currentPage, itemsPerPage, filterRating, filterDoctor]);
  ...
  const totalPages = Math.ceil((allFeedback?.total || 0) / itemsPerPage);

  // Get unique doctors for filter
  const uniqueDoctors = allFeedback?.feedback?.reduce((acc, feedback) => {
    if (feedback.doctor && !acc.find(d => d._id === feedback.doctor._id)) {
      acc.push(feedback.doctor);
    }
    return acc;
  }, []) || [];
  ...
  return (
    <div className="p-6 space-y-6">
          ...
          <div className="bg-white rounded-lg shadow p-6">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <p className="text-2xl font-bold text-gray-900">{allFeedback.totalFeedback || 0}</p>
              </div>
            </div>
          </div>
            ...
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <p className="text-2xl font-bold text-gray-900">
                  {allFeedback.averageRating ? allFeedback.averageRating.toFixed(1) : '0.0'}
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-lg shadow p-6">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <p className="text-2xl font-bold text-gray-900">{allFeedback.fiveStarCount || 0}</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-lg shadow p-6">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <p className="text-2xl font-bold text-gray-900">{allFeedback.lowRatingCount || 0}</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="bg-white rounded-lg shadow">
        ...
        ) : allFeedback?.feedback?.length > 0 ? (
          <div className="divide-y divide-gray-200">
            {allFeedback.feedback.map((feedback) => (
                  ...
                  <div className="flex-1">
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex items-center text-sm text-gray-600">
                        ...
                        <button
                          onClick={() => handleViewProfile(feedback.doctor._id, 'doctor')}
                          className="ml-1 text-blue-600 hover:text-blue-800 underline font-medium"
                        >
                          {feedback.doctor?.username || 'Bác sĩ'}
                        </button>
                      </div>
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2 text-sm">
                ...
                <div className="flex items-center">
                  <span className="font-medium">Bác sĩ: </span>
                  <button
                    onClick={() => handleViewProfile(selectedFeedback.doctor._id, 'doctor')}
                    className="ml-1 text-blue-600 hover:text-blue-800 underline font-medium"
                  >
                    {selectedFeedback.doctor?.username || 'Bác sĩ'}
                  </button>
                </div>
